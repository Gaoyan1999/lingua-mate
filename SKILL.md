...
If the user provides a Bilibili URL instead of a local file, ask for the learner's English level as early as possible, before validating the URL in detail or starting any download. Then confirm it is a concrete `/video/BV...` or `/video/av...` link and continue with the normal media pipeline.

If the user provides a YouTube URL instead of a local file, ask for the learner's English level as early as possible, before validating the URL in detail or starting any download. Then confirm it is a concrete watch, `youtu.be`, Shorts, embed, or live video link and continue with the normal media pipeline.
...
1. Ask the user for the learner's English level if it was not already provided. This is the first user input to collect after receiving the media source, including Bilibili and YouTube links. Keep the question simple: "What's your English level? You can answer beginner, intermediate, advanced, or tell me your test/school level." Also accept CEFR labels such as `A2`, `B1`, `B2`, or `C1` if the user provides them. Stop and wait for the user's answer before detailed URL validation, any download, transcription, translation, `readThrough`, or `vocabulary` work; do not infer a default level.
2. Determine whether the source material is a local file, a concrete Bilibili video link, or a concrete YouTube video link.
...
1. Ask for the learner's English level if not already known, and wait for the user's answer before URL validation, download, transcription, or generation. Do this as early as possible after the user provides the media source. Do not choose a default level. Use this simple question: "What's your English level? You can answer beginner, intermediate, advanced, or tell me your test/school level." Use the answer for study-note difficulty and density:
...
3. If the source is a Bilibili or YouTube video link, download it only after the learner level is confirmed.
